...
>In this department each student is supplied with a set of tools, most of which are new. These tools are kept, while not in use, in a pretty glass case just over each bench. Such tools as are not used much are obtained by checks, thus making each student individually responsible for tools used.(l) 
